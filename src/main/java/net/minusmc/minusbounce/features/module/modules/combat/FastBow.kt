...
class FastBow : Module() {
        ...
        if (itemStack.item is ItemBow) {
            ...
            when (modeValue.get().lowercase()) {
                "normal" -> {
                    if (delay.get() == 0) {
                        repeat (packetsValue.get()) {
                            PacketUtils.sendPacketNoEvent(C05PacketPlayerLook(rotation.yaw, rotation.pitch, true))
                        }
                ...
                "1.17" -> {
                    ...
                    repeat(packetsValue.get()) {
                        PacketUtils.sendPacketNoEvent(C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, rotation.yaw, rotation.pitch, mc.thePlayer.onGround))
                    }
                }
            }
    ...
    fun onPacket(event: PacketEvent) {
        if (modeValue.get().equals("1.17", true))
            return
        ...
        if ((packet is C08PacketPlayerBlockPlacement || (packet is C07PacketPlayerDigging && packet.status == C07PacketPlayerDigging.Action.RELEASE_USE_ITEM)) && itemStack.item is ItemBow)
            event.cancelEvent()
    }
}