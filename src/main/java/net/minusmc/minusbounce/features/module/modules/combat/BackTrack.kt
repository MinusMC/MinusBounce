...
class BackTrack : Module() {
    val modeValue = ListValue("TrackMode", arrayOf("PacketDelay", "Automatic", "Manual"), "Automatic")
    ...
    private val sizeValue = IntegerValue("Delay", 100, 0, 1000).displayable { modeValue.equals("PacketDelay") }
    ...
    fun onPacket(event: PacketEvent) {
        if (modeValue.equals("PacketDelay")) {
            ...
            if (packet.javaClass.name.startsWith("net.minecraft.network.play.server.")) {
                ...
                val place: C08PacketPlayerBlockPlacement = packet
    ...
    @EventTarget
    fun onMotion(event: PostMotionEvent) {
        if (modeValue.equals("PacketDelay")) {
            if (packets.isEmpty()) return;
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        if (modeValue.equals("PacketDelay")) {
            for (data in entityPosMap.values) {
                if (MinusBounce.combatManager.target != null) {
                    ...
                    if (MinusBounce.combatManager.target!!.hurtTime > 0) RenderUtils.glColor(
                        255,
                        32,
                        32,
                        35
        ...
        while (storagePackets.isNotEmpty()) {
            storagePackets.removeAt(0).let {
                try {
                    ...
                    if (!packetEvent.contains(it)) MinusBounce.eventManager.callEvent(packetEvent)
